...
export async function sendSMS(env, to, text) {
  ...
  // Route delivery/status webhooks to a safe endpoint (not inbound)
  if (env.STATUS_WEBHOOK_URL) {
    body.webhook_url = env.STATUS_WEBHOOK_URL;   // e.g. https://easyforty.com/telnyx/status?token=XYZ
  }
