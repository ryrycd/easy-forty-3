...
export async function onRequestPost({ request, env }) {
  ...
  if (!cfgKey) {
    return new Response('Server not configured: ADMIN_KEY is missing in environment variables. In Cloudflare Pages, set it for PRODUCTION and PREVIEW, then redeploy.', { status: 500 });
  }
  const key = request.headers.get('X-Admin-Key');
  if (!key || key !== cfgKey) {
    return new Response('Unauthorized: Invalid admin key. Tip: Verify ADMIN_KEY in Pages → Settings → Environment variables for this deployment.', { status: 401 });
  }
