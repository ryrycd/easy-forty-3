...
import { getLinks, setLinks, getUser, setUser, pickActiveLink, sendSMS, logEvent, rotatorExhausted } from '../../lib/store.js';
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!validHandle(handle)) return json({ error: 'Invalid payout handle.' }, 400);
    ...
    if (!choice) {
      return json({ error: 'All referral links are currently exhausted. Try again later.' }, 503);
    }
